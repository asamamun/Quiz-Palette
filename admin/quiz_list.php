...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_quiz'])) {
    ...
    try {
        ...
        if (!isset($_POST['question_id'], $_POST['question'], $_POST['status'], 
                  $_POST['option_a_id'], $_POST['option_a'], 
                  $_POST['option_b_id'], $_POST['option_b'], 
                  $_POST['option_c_id'], $_POST['option_c'], 
                  $_POST['option_d_id'], $_POST['option_d'], 
                  $_POST['correct_option'], $_POST['category_id'], $_POST['subject_id'], $_POST['quiz_id'])) {
        ...
        if (!in_array($status, $allowed_statuses)) {
            throw new Exception("Invalid status value: " . htmlspecialchars($status));
        }

        // Update question in the questions table
        $stmt = $conn->prepare("UPDATE questions SET question_text=?, status=? WHERE id=?");
        $stmt->bind_param("ssi", $_POST['question'], $status, $_POST['question_id']);
        if (!$stmt->execute()) {
            throw new Exception("Failed to update question or status: " . $stmt->error);
        }
        ...
        foreach ($options as $option) {
            ...
            if (!$stmt->execute()) {
                throw new Exception("Failed to update option ID {$option['id']}: " . $stmt->error);
            }
        }
        ...
        if (!$stmt->execute()) {
            throw new Exception("Failed to update quiz metadata: " . $stmt->error);
        }
        ...
        $conn->rollback();
        error_log("Edit error: " . $e->getMessage()); // Log error for debugging
...
if (isset($_GET['delete'])) {
    ...
    if ($question_id <= 0) {
        header("Location: quiz_list.php?error=Invalid question ID");
        exit;
    }
    $conn->begin_transaction();
    try {
...
$sql = "SELECT q.id AS quiz_id, q.title, q.status, q.category_id, q.subject_id, 
               ...
               ques.status AS question_status,
        ...
        ORDER BY ques.id ASC";
$questions = $conn->query($sql);
if (!$questions) {
    error_log("Query error: " . $conn->error); // Log query error
}
?>
        ...
        <div class="col-md-10 col-12 p-0">
                ...
                <table id="quizTable" class="table table-bordered table-striped table-hover">
                    ...
                    <tbody>
                        <?php while ($q = $questions->fetch_assoc()): ?>
                            <tr>
                                <td class="dt-hidden"></td> <!-- Serial number for export only -->
                                ...
                                <td><?= htmlspecialchars(ucfirst($q['question_status'] ?? 'Active')) ?></td>
                                <td>
                                    <button
                                        class="btn btn-sm btn-warning editBtn"
                                        data-question_id="<?= htmlspecialchars($q['question_id']) ?>"
                                        data-quiz_id="<?= htmlspecialchars($q['quiz_id']) ?>"
                                        ...
                                        data-option_a_id="<?= htmlspecialchars($q['option_a_id'] ?? '') ?>"
                                        data-option_b_id="<?= htmlspecialchars($q['option_b_id'] ?? '') ?>"
                                        data-option_c_id="<?= htmlspecialchars($q['option_c_id'] ?? '') ?>"
                                        data-option_d_id="<?= htmlspecialchars($q['option_d_id'] ?? '') ?>"
                                        data-correct_option="<?= htmlspecialchars($q['is_correct_a'] ? 'a' : ($q['is_correct_b'] ? 'b' : ($q['is_correct_c'] ? 'c' : ($q['is_correct_d'] ? 'd' : '')))) ?>"
                                        data-status="<?= htmlspecialchars($q['question_status'] ?? 'active') ?>"
                                        data-category_id="<?= htmlspecialchars($q['category_id'] ?? '') ?>"
                                        data-subject_id="<?= htmlspecialchars($q['subject_id'] ?? '') ?>"
                                    >Edit</button>
                                    <a href="?delete=<?= htmlspecialchars($q['question_id']) ?>" class="btn btn-sm btn-danger" onclick="return confirm('Are you sure you want to delete this question?')">Delete</a>
                                </td>
                            </tr>
                        <?php endwhile; ?>
                    </tbody>
            ...
            <div class="modal fade" id="editModal" tabindex="-1" aria-labelledby="editModalLabel" aria-hidden="true">
                <div class="modal-dialog modal-lg">
                    <form method="POST" action="quiz_list.php" class="modal-content">
                        ...
                        <div class="modal-body">
                            <div class="mb-3">
                                ...
                                <textarea name="question" id="edit_question" class="form-control"></textarea>
                            </div>
                            <div class="row g-3">
                                <div class="col-md-6">
                            ...
                            <div class="row g-3 mt-3">
                                <div class="col-md-4">
                                    ...
                                    <select name="correct_option" id="edit_correct_option" class="form-select">
                                ...
                                <div class="col-md-4 mt-3">
                                    ...
                                    <select name="subject_id" id="edit_subject_id" class="form-select">
                                        ...
                                        while ($sub = $subjects->fetch_assoc()) {
                                            echo "<option value='".htmlspecialchars($sub['id'])."'>" . htmlspecialchars($sub['name']) . "</option>";
                                        }
                                        ?>
                                    </select>
                                </div>
...
$(document).ready(function () {
    // Clear query parameters after displaying alerts
    ...
    if (!$.fn.DataTable.isDataTable('#quizTable')) {
            ...
            buttons: [
                {
                    ...
                    exportOptions: {
                        columns: [0, 2, 3, 4] // S.No, Question, Options, Correct Option
                    },
        ...
        $('#quizTable').on('click', '.editBtn', function (e) {
            e.preventDefault();
            try {
                const btn = $(this);
                ...
                console.error('Error in edit button handler:', error.message, error.stack);
            }
        });
    }
});
