...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['add_exam'])) {
        ...
        if ($stmt->execute()) {
            ...
            // Insert quiz associations
            $stmt = $conn->prepare("INSERT INTO exam_quizzes (exam_id, quiz_id) VALUES (?, ?)");
            while ($quiz = $result->fetch_assoc()) {
                $stmt->bind_param("ii", $exam_id, $quiz['id']);
                $stmt->execute();
            }
            echo "<div class='alert alert-success alert-dismissible fade show' role='alert'>Exam added with " . $result->num_rows . " quizzes.<button type='button' class='btn-close' data-bs-dismiss='alert'></button></div>";
        } else {
